...
MaxCliqueVisitor::MaxCliqueVisitor(ClusterMeans const& clusterizer,
    ...
    : clusterizer_(clusterizer), max_covered_points_(max_covered_points), max_clique_idx_(max_clique_idx) {}
...
std::vector<Eigen::Index> findMaxClique(ClusterMeans const& clusterizer, double eps) {
  ...
  for (Eigen::Index i = 0; i < clusterizer.getClusterNumber(); ++i) {
    for (Eigen::Index j = 0; j < i; ++j) {
      if (std::abs(clusterizer.getMeans().col(i).dot(clusterizer.getMeans().col(j))) < eps) {
