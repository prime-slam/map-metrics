...
ClusterMeans::ClusterMeans(alglib::integer_1d_array const& labels, Eigen::Index cluster_number)
    : cluster_number_(cluster_number), labels_(Eigen::VectorXi(labels.length())), cluster_means_(), cluster_idx_() {
...
const Eigen::VectorXi& ClusterMeans::getLabels() const { return this->labels_; }
