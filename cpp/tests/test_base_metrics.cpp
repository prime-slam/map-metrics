...
std::vector<std::vector<double>> GetCalibrationMatrix() {
  return {{4.276802385584e-04, -9.999672484946e-01, -8.084491683471e-03, -1.198459927713e-02},
...
TEST(LidarTest, MPV) {
  ...
  auto config = map_metrics::Config(5, 1.0, 30, 5);
  auto points = map_metrics::aggregateMap(std::vector<cilantro::VectorSet3d>(PCs.begin(), PCs.begin() + 5),
                                          std::vector<Eigen::Matrix4d>(tj_gt.begin(), tj_gt.begin() + 5));
  auto points_map = map_metrics::MapTree(points, config.knn_rad);
  double actual_mpv = map_metrics::MPV(points_map, config.min_knn);
  ...
  ASSERT_LE(std::fabs(actual_mpv - 0.019802606170394272), 1e-8);
}
...
TEST(LidarTest, MME) {
  ...
  auto points = map_metrics::aggregateMap(std::vector<cilantro::VectorSet3d>(PCs.begin(), PCs.begin() + 5),
                                          std::vector<Eigen::Matrix4d>(tj_gt.begin(), tj_gt.begin() + 5));
  auto points_map = map_metrics::MapTree(points, config.knn_rad);
  double actual_mme = map_metrics::MME(points_map, config.min_knn);
  auto elapsed_milliseconds =
      std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - start_time);
  ...
  ASSERT_LE(std::fabs(actual_mme - 0.018431360563601673), 1e-8);
}
